...
ROOT = Path("<ORIGINAL_PROJECT_ROOT>/study_results")
OUTDIR = ROOT / "axisA_bootstrap_auc_ci_NUEVOS_axisA_only"
FINAL_MODELING = ROOT / "NUEVOS" / "MODELING"
FINAL_EXTERNAL = ROOT / "NUEVOS" / "extvalidation"
N_BOOT = 10_000
SEED = 1337
ARMS = ("NR", "RS", "VS", "FK")
MODELS = ("LR", "XGB", "MLP")
...
def prediction_source(spec: CohortSpec, arm: str, model: str) -> Path:
    if spec.validation == "internal held-out":
        row = internal_result_row(spec, arm, model)
        return Path(str(row["preds_path"]))
